...
async def load_citizenship(message: types.Message, state: FSMContext):
    ...
    if message.text in choices.name_citizenship:
        await bot.send_message(chat_id=message.from_user.id, text='<em>Your datas for cita</em>', reply_markup=ReplyKeyboardRemove(), parse_mode='HTML')
        await bot.send_message(chat_id=message.from_user.id,
            text=f"\n\n<b>{data['province']}\n{data['department']}\n{data['reason']}\n{data['numb_doc']}\n{data['name_sername']}\n{data['age']}\n{data['nation']}</b>\n\n<em>Has been save\n\nPlease check your details and confirm the start of the search for vacancies</em>",
            reply_markup=kb_sel.ikb_save_pogress(),
            parse_mode='HTML')
